...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const initialAuthMode = (localStorage.getItem('authMode') as AuthMode) || 'LOCAL';
  ...
  useEffect(() => {
    ...
    if (authMode === 'LOCAL') {
      ...
      console.log(`[AuthContext] Loaded token from localStorage: ${localToken}`);
    } else if (authMode === 'SESSION') {
      const sessionToken = sessionStorage.getItem('token');
      setTokenState(sessionToken);
      console.log(`[AuthContext] Loaded token from sessionStorage: ${sessionToken}`);
    } else {
      setTokenState(null);
  ...
  const setToken = (newToken: string) => {
    console.log(`[AuthContext] Setting token: ${newToken} for mode: ${authMode}`);
    if (authMode === 'LOCAL') {
  ...
  return (
    <AuthContext.Provider value={{ token, authMode, setAuthMode, setToken, clearToken }}>
      {children}
